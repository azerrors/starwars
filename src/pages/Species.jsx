...
import { getSearchedSpeciesData } from "../services/useApi";
...
function Species() {
  ...
  //getting speacies datas from useApi.jsx
  const { data: searchedSpecies, status: speciesLoading } = useQuery({
    keepPreviousData: true,
    queryKey: ["planet", speciesInput, page],
    queryFn: () => getSearchedSpeciesData(speciesInput, page),
  });
      ...
      {speciesLoading === "loading" ? (
        ...
        <>
          {searchedSpecies.length === 0 ? (
            <NotFoundMessage>
              There Is Nothing To Show About `{speciesInput}`. Search Something
              Else
            </NotFoundMessage>
          ) : (
            <>
              <List species={searchedSpecies} />
